...
class NCTAnomalyDetectorV2:
    ...
    def calibrate(self, normal_loader: torch.utils.data.DataLoader, percentile: float = 75.0, use_median: bool = True):
        """
        在正常数据上校准阈值
        
        Args:
            normal_loader: 正常数据的 DataLoader
            percentile: 百分位数（如 75 表示 top 25% 为异常）
            use_median: 是否使用中位数而非百分位数
        """
        self.model.eval()
        ...
        all_pes = np.array(all_pes)
        all_phis = np.array(all_phis)
        all_entropies = np.array(all_entropies)
        
        # Set thresholds using median or percentile
        if use_median:
            # 使用中位数 + 标准差的方法
            self.pe_threshold = np.median(all_pes) + 1.5 * np.std(all_pes)
            self.phi_threshold = np.median(all_phis) - 1.5 * np.std(all_phis)  # Low Φ is anomalous
            self.entropy_threshold = np.median(all_entropies) + 1.5 * np.std(all_entropies)
            
            print(f"Using median-based thresholds (median ± 1.5*std):")
        else:
            # 使用百分位数
            ...
            print(f"Using percentile-based thresholds ({percentile}):")
        
        # Save statistics
        self.stats = {
            'pe_mean': float(np.mean(all_pes)),
            'pe_std': float(np.std(all_pes)),
            'pe_median': float(np.median(all_pes)),
            'phi_mean': float(np.mean(all_phis)),
            'phi_std': float(np.std(all_phis)),
            'phi_median': float(np.median(all_phis)),
        ...
        print(f"  Φ threshold: {self.phi_threshold:.6f} (<{self.phi_threshold:.6f} → anomaly)")
