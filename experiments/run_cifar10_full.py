...
def get_args():
    ...
    # 测试模式
    parser.add_argument('--skip_training', action='store_true', help='Skip training and directly evaluate')
    parser.add_argument('--test_model', type=str, default=None, help='Path to trained model for testing (required when --skip_training)')
...
def main():
    ...
    if args.skip_training:
        if not args.test_model:
            raise ValueError("--test_model is required when using --skip_training")
        
        print("\n" + "=" * 60)
        print("TEST MODE: Skip training, directly evaluate")
        print("=" * 60)
        ...
        # Load test model
        print(f"\nLoading model from {args.test_model}...")
        checkpoint = torch.load(args.test_model, map_location=args.device)
        ...
        test_loss, test_acc, test_phi, test_pe = evaluate(
        ...
        print(f"  Test Φ: {test_phi:.4f}")
        print(f"  Test PE: {test_pe:.4f}")
        
        # Also evaluate on training set for comparison
        print("\n" + "=" * 60)
        print("Testing on Training Set (for reference)")
        print("=" * 60)
        
        train_loss, train_acc, train_phi, train_pe = evaluate(
        ...
        print(f"  Train Φ: {train_phi:.4f}")
        print(f"  Train PE: {train_pe:.4f}")
        ...
        test_results = {
            ...
            'test_phi': float(test_phi),
            'test_pe': float(test_pe),
            'train_loss': float(train_loss),
            'train_acc': float(train_acc),
            'train_phi': float(train_phi),
            'train_pe': float(train_pe),
            'config': vars(args)
        }
        ...
        print(f"\nTest results saved to: {results_dir / 'test_results.json'}")
        print("\n" + "=" * 80)
    ...
    # Save training history - use custom encoder for numpy types
    results_summary = {
        'config': config,
